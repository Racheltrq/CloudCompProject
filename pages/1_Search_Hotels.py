...
def search():
  ...
  for hotel in results:
    ...
    # Add some elements to the container
      ...
      #st.markdown('<a href="/Book_Hotel" target="_self" style="background-color: #ffffff; border: 1px solid rgba(49, 51, 63, 0.2); border-radius: 0.25rem; color: black; padding: 0.25rem 0.75rem; text-align: center; text-decoration: none;display: inline-block; font-weight: 400; font-size: 0.9rem;">Book</a>', unsafe_allow_html=True)
      st.form_submit_button(label="Choose", on_click=set_state, args=(hotel['name'],))
    count += 1
  #st.write(results)
